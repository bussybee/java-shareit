...
public class ItemController {
    ...
    @PostMapping
    public ResponseEntity<ItemDto> create(@RequestBody @Valid ItemDto item, @RequestHeader("X-Sharer-User-Id") Long userId) {
        log.info("Creating new item: {}", item);
        return new ResponseEntity<>(itemService.create(item, userId), HttpStatus.CREATED);
    }

    @PatchMapping("/{id}")
    public ResponseEntity<ItemDto> update(@RequestBody @Valid ItemUpdateRequestDto item, @PathVariable Long id, @RequestHeader("X-Sharer-User-Id") long userId) {
        log.info("Updating existing item: {}", item);
        return new ResponseEntity<>(itemService.update(item, id, userId), HttpStatus.OK);
    }

    @GetMapping("/{id}")
    public ResponseEntity<ItemDto> getById(@PathVariable Long id, @RequestHeader("X-Sharer-User-Id") Long userId) {
    ...
    @GetMapping("/search")
    public ResponseEntity<List<ItemDto>> searchByText(@RequestParam String text, @RequestHeader("X-Sharer-User-Id") Long userId) {
